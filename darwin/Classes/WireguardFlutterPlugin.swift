...
class VPNUtils {
    ...
    NSLog("Statistics IOS")
    loadProviderManager { error in
        if let error = error {
        ...
        do {
            NSLog("Sending GET_STATISTICS message to tunnel provider")
            try session.sendProviderMessage("GET_STATISTICS".data(using: .utf8)!) { response in
                guard let response = response,
                      let responseString = String(data: response, encoding: .utf8) else {
